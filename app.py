        ...
        if st.button("Начать анализ", use_container_width=True, type="primary"):
            with st.spinner("ИИ проводит глубокий юридический аудит..."):
                ...
                prompt_instruction = (
                    ...
                    "[PAYWALL]\n" # <--- ДОБАВЬТЕ ЭТО МЕЖДУ АНАЛИЗОМ И ТАБЛИЦЕЙ
                ...
                # 1. Сначала мы просто сохраняем ответ от ИИ в переменную
                full_text = response.choices[0].message.content

                # 2. ДИАГНОСТИКА (Временно добавь эту строку!)
                # st.write("Отладка: Проверяю наличие тега...", "[PAYWALL]" in full_text)

                # 3. ЛОГИКА ОТОБРАЖЕНИЯ (Оставляем только этот блок!)
                if "[PAYWALL]" in full_text:
                    # Режем текст на До и После
                    ...
                    if is_paid:
                        ...
                        st.warning("⚠️ Таблица с правками скрыта.")
                        st.button("Купить Premium за 850 ₽")
                
                else:
                    # Если тега нет — выводим текст и ругаем ИИ (для теста)
                    st.error("Ошибка: ИИ не добавил разделитель [PAYWALL]. Попробуйте еще раз.")
