...
with tab_demo:
    st.markdown("### 📄 Пример идеального отчета")
    st.write("Вот как будет выглядеть ваш анализ после оплаты:")
    st.error("🔴 **Критический риск:** Пункт 5.3 позволяет изъять имущество без суда.")
    st.warning("🟠 **Средний риск:** Нечетко прописаны сроки возврата депозита.")
    st.success("🟢 **Рекомендация:** Добавьте пункт о форс-мажорных обстоятельствах.")
